...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // Redemptions use the canonical in-memory runtime shared with /api/redemptions.
    const redemptionState = (globalThis as any).__redemptions;
    const redemptions = Number(redemptionState?.byCampaign?.[campaignId] ?? 0);
