...
export default function Page() {
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem(LS_PERIOD, String(period));
    } catch {}
  }, [period]);
  ...
  const pushQS = React.useCallback(
    (e: string, a: boolean, p: number) => {
  ...
  const csvHref = React.useMemo(
    () => `/api/lumaspace/shadow/export?email=${encodeURIComponent(email || "")}`,
    [email]
  );
  ...
  return (
    <main style={wrap}>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 10 }}>
        <Link href="/lumaspace" style={backLink}>
          ← LumaSpace
        </Link>
      </div>
      ...
      <div style={toolbar}>
        <label htmlFor="email" style={{ fontWeight: 800, marginRight: 4 }}>
          Email
        </label>
        <input
          id="email"
        ...
        <a href={csvHref} target="_blank" rel="noopener" style={btnGhost}>
          Export CSV
        </a>
        ...
        <label htmlFor="autoSel" style={{ display: "inline-flex", alignItems: "center", gap: 6, marginLeft: 10, fontWeight: 800 }}>
          <input
            id="autoChk"
            type="checkbox"
            checked={auto}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
              ...
              pushQS(email, e.target.checked, period);
            }}
        ...
        <select
          ...
          onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
            const v = clampPeriod(Number(e.target.value));
            setPeriod(v);
            pushQS(email, auto, v);
          }}
          disabled={!auto}
...
const stamp: React.CSSProperties = { fontSize: 12, opacity: 0.7, marginLeft: 6, fontWeight: 700 };