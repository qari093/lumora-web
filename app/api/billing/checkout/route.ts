...
export async function POST(req: NextRequest) {
  ...
  if (!ownerId || !isFinite(euros) || euros <= 0) {
    return NextResponse.json(
      { ok: false, error: 'BAD_REQUEST', need: ['ownerId', 'euros>0'], requestId: id },
    ...
    return NextResponse.json(
      {
        ...
        message: 'Install stripe: npm i stripe',
        requestId: id,
      },
      { status: 200, headers: { 'x-request-id': id } },
    );
  ...
  try {
      ...
      line_items: [
        {
          price_data: {
            currency: 'eur',
            product_data: { name: `Lumora Credits – €${euros.toFixed(2)}` },
            unit_amount: amount,
          },
          quantity: 1,
        },
    ...
    return NextResponse.json(
      { ok: true, url: session.url, id: session.id, requestId: id },
      { status: 200, headers: { 'x-request-id': id } },
    );
  } catch (e: any) {
    return NextResponse.json(
      { ok: false, error: 'STRIPE_ERROR', message: String(e?.message || e), requestId: id },
      { status: 200, headers: { 'x-request-id': id } },
    );
  }
}
