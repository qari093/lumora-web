...
export async function GET() {
  ...
  const emotionTags = Array.isArray(first?.derivedEmotionTags)
    ? first.derivedEmotionTags
    : Array.isArray(first?.emotionTags)
      ? first.emotionTags
      : ['curiosity'];
  const content = attachEmotionTags({ ...base, tags: emotionTags });
