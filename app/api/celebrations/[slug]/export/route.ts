...
export async function GET(_req: Request, ctx: { params: { slug: string } }) {
  ...
  const [parts, reacts, rewards] = await Promise.all([
    prisma.celebrationParticipant.findMany({
      where: { celebrationId: c.id },
      orderBy: { createdAt: 'asc' },
    }),
    prisma.celebrationReaction.findMany({
      where: { celebrationId: c.id },
      orderBy: { createdAt: 'asc' },
    }),
    prisma.celebrationReward.findMany({
      where: { celebrationId: c.id },
      orderBy: { createdAt: 'asc' },
    }),
  ]);
  ...
  for (const p of parts)
    rows.push(
      ['participants', p.id, p.userId ?? '', '', '', p.createdAt.toISOString()].map(esc).join(','),
    );
  ...
  for (const r of rewards)
    rows.push(
      ['rewards', r.id, r.userId ?? '', r.type ?? '', '', r.createdAt.toISOString()]
        .map(esc)
        .join(','),
    );
