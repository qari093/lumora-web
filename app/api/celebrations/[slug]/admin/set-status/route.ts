...
import { requireApiAdmin } from '@/lib/auth/requireApiRole';
const prisma = new PrismaClient();

export const dynamic = 'force-dynamic';

export async function POST(req: Request, ctx: { params: { slug: string } }) {
  const authorizationFailure = await requireApiAdmin();
  ...
  try {
    ...
    if (!c)
      return NextResponse.json({ ok: false, error: 'celebration not found' }, { status: 404 });
