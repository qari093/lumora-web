...
export default function ShadowPage() {
  ...
  async function load() {
    try {
      const r = await fetch(`/api/lumaspace/shadow?email=${encodeURIComponent(email)}&take=20`, {
        cache: 'no-store',
      });
  ...
  return (
    <main style={{ padding: 20, maxWidth: 860, margin: '0 auto' }}>
      ...
      <p style={{ opacity: 0.8, marginBottom: 14 }}>Private stream for quick emotional notes.</p>
      ...
      >
        ...
        <button onClick={add} disabled={loading} style={btnStyle}>
          {loading ? 'Saving…' : 'Add'}
        </button>
        <button onClick={load} style={{ ...btnStyle, background: '#09f', borderColor: '#09f' }}>
          Refresh
        </button>
      </div>
          ...
          {items.map((e) => (
            ...
            >
              ...
              <div style={{ marginTop: 6, opacity: 0.85 }}>
                Mood: {e.emotion || '—'} • Privacy: {e.privacy || '—'}
              </div>
