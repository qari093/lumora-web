...
export default function ShowsPanel({ slug, apiBase }: { slug: string; apiBase: string }) {
  ...
  async function createShow() {
    ...
    try {
      ...
      const j = await r.json();
      if (!r.ok) {
        alert(j?.message || j?.error || 'Create failed');
        return;
      }
      setTitle('');
      setDate('');
  ...
  function beginEdit(sh: Show) {
    ...
    const dur = Math.max(
      1,
      Math.round((new Date(sh.endAt).getTime() - new Date(sh.startAt).getTime()) / 60000),
    );
    setEditDuration(dur);
  }
  ...
  async function saveEdit(id: string) {
    ...
    try {
      ...
      const j = await r.json();
      if (!r.ok) {
        alert(j?.message || j?.error || 'Update failed');
        return;
      }
      setEditingId(null);
      await refresh();
  ...
  const formRow = {
    display: 'grid',
    gridTemplateColumns: '180px 1fr 1fr 130px auto',
    gap: 10,
    alignItems: 'center',
  } as const;
  ...
  return (
    <section style={{ marginTop: 16 }}>
      <div style={{ display: 'inline-flex', gap: 8, marginTop: 8 }}>
        <button
          data-cy="export-shows-csv"
          onClick={() => openShowsCsv(slug)}
          style={{
            padding: '8px 12px',
            borderRadius: 10,
            border: '1px solid rgba(255,255,255,.2)',
      ...
      <h3 style={{ fontSize: 16, opacity: 0.9 }}>Shows — schedule</h3>
      ...
      <div style={{ ...formRow, marginTop: 10 }}>
        ...
        <input
          ...
          min={1}
          value={duration}
          onChange={(e) => setDuration(parseInt(e.target.value || '0', 10) || 45)}
        />
        <button disabled={loading} onClick={createShow} style={btn}>
          Add
        </button>
      </div>
            ...
            {editingId === sh.id ? (
              <>
                <div style={{ ...formRow, marginTop: 6 }}>
                  ...
                  <input
                    ...
                    min={1}
                    value={editDuration}
                    onChange={(e) => setEditDuration(parseInt(e.target.value || '0', 10) || 45)}
                  />
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button disabled={loading} onClick={() => saveEdit(sh.id)} style={btn}>
                      Save
                    </button>
                    <button onClick={() => setEditingId(null)} style={btn}>
                      Cancel
                    </button>
                  </div>
                </div>
              ...
              <>
                ...
                <div style={{ marginTop: 6, fontSize: 13, opacity: 0.9 }}>
                  {new Date(sh.startAt).toLocaleString()} →{' '}
                  {new Date(sh.endAt).toLocaleTimeString()}
                </div>
                ...
                <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
                  <button onClick={() => beginEdit(sh)} style={btn}>
                    Edit
                  </button>
                  <button onClick={() => removeShow(sh.id)} style={btn}>
                    Delete
                  </button>
                </div>
              </>
