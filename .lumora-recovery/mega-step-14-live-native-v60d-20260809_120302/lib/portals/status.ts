...
export function allPortalsAlive(): {
  ok: boolean;
  portals: ReadonlyArray<PortalAliveItem>;
  ts: number;
} {
...
export const PORTAL_STATUS: Record<
  string,
  { ok: boolean; status: 'alive' | 'down' | 'disabled'; label: string }
> = Object.fromEntries(
  _getRegistry()
    .map((p) => {
...
export type PortalStatus = 'active' | 'seed' | 'inactive' | 'alive' | 'down' | 'disabled';

export type PortalStatusGridRow = Readonly<{
  key: string;
  label: string;
  url: string;
  ok: boolean;
...
export async function getPortalStatusGrid(): Promise<PortalStatusGridRow[]> {
  return portalAliveSnapshot().map(
    (portal): PortalStatusGridRow => ({
      key: String(portal.key),
      label: String(portal.key),
      url: String(portal.route),
      ok: portal.ok,
      status: portal.ok ? 200 : 0,
      ms: 0,
    }),
  );
}
