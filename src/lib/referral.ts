...
async function uid(): Promise<string> {
  ...
  let id = jar.get('lumora_uid')?.value;
...
function codeFor(u: string) {
  return u.replace(/-/g, '').slice(0, 8).toUpperCase();
}
export async function issueReferral() {
  const id = await uid();
  return { code: codeFor(id) };
}
export async function claimReferral(code: string) {
  ...
  if (r) {
    ...
    if (!owner) {
      await r.set(ownerKey, id, 'EX', 365 * 24 * 3600);
      return { ok: false as const, error: 'self_code_init' };
    }
    ...
    return {
      ...
      addedOwner: 5,
      addedJoiner: 3,
      ownerTotal: ownerEarn,
      joinerTotal: joinEarn,
    };
  }
  return {
    ok: true as const,
