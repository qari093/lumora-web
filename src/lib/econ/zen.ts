...
// LUMORA_SAFE_READFILESYNC_ENOENT_V1
function __lumoraSafeReadFileSync(fn: () => string): string {
  try { return fn(); }
  catch (e: any) {
...
function load():Z{ try{ return JSON.parse(__lumoraSafeReadFileSync(() => fs.readFileSync(P,"utf8"))); }catch{ return { balance:0 }; } }
...
export async function snapshot(){ try{ const j=JSON.parse(__lumoraSafeReadFileSync(() => fs.readFileSync(P,"utf8"))); return { balance: Number(j?.balance||0) }; } catch { return { balance: 0 }; } }
