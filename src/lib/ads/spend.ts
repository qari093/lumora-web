...
export async function charge(
  ...
  if (campaignId) {
    const c = (await getCampaign(campaignId)) as { budgetEuros: number; state?: string } | null;
    if (!c)
      return {
        charged: false as const,
        reason: 'NO_SUCH_CAMPAIGN' as const,
        amount,
        balance: getWallet(ownerId).euros,
      };
    const spent = await computeCampaignSpendEuros(campaignId);
    if (spent >= c.budgetEuros) {
      // auto-pause campaign
      await setState(campaignId, 'pause');
      return {
        charged: false as const,
  ...
  try {
    const reason = campaignId
      ? `ads:${kind}:c:${creativeId || ''}:cmp:${campaignId}`
      : `ads:${kind}`;
    ...
    if (thr !== null && wallet.euros < thr) {
      emitNotification(
        ownerId,
        'low_balance',
        `Low balance: €${wallet.euros.toFixed(2)} (< €${thr.toFixed(2)})`,
        'Top up to keep campaigns running',
        { balance: wallet.euros, threshold: thr },
      );
    }
    ...
    if (campaignId) {
      const c = (await getCampaign(campaignId)) as { budgetEuros: number; state?: string } | null;
      if (c) {
        const spentNow = await computeCampaignSpendEuros(campaignId);
