...
function bKey(uid: string, type: string) {
  return uid + '::' + type;
}
...
export function limit(uid: string, type: 'WATCH' | 'LIKE' | 'COMMENT' | 'SHARE') {
  ...
  const cfg =
    type === 'WATCH'
      ? { ratePerSec: 40 / 60, burst: 40 }
      : type === 'SHARE'
        ? { ratePerSec: 10 / 60, burst: 10 }
        : { ratePerSec: 30 / 60, burst: 30 };
  const key = bKey(uid, type);
  let b = buckets.get(key);
  if (!b) {
    b = { tokens: cfg.burst, last: Date.now() / 1000, ...cfg };
    buckets.set(key, b);
  }
  refill(b);
  if (b.tokens >= 1) {
...
export function issueToken(uid: any) {
  ...
  if (uid && typeof uid === 'object') {
    ...
    if (typeof ttlMsIn === 'number' && Number.isFinite(ttlMsIn))
      ttlMs = Math.max(1, Math.floor(ttlMsIn));
    else if (typeof ttlSec === 'number' && Number.isFinite(ttlSec))
      ttlMs = Math.max(1, Math.floor(ttlSec * 1000));
  } else {
    outUid = String(uid ?? '');
  }
...
export function verifyToken(token: string, maxAgeMs = 60_000) {
  ...
  if (!uid || !Number.isFinite(ts) || !jti || !sig)
    return { ok: false, error: 'bad_token' } as const;
  ...
  if (!crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(expSig)))
    return { ok: false, error: 'bad_sig' } as const;
