...
type TwitterXIngestOptions = {
  limit?: number;
  useFixtureOnFailure?: boolean;
};

const FIXTURE_PATH = path.join(process.cwd(), "src/lib/signals/fixtures/twitter_x.sample.json");
...
function normalizeTwitterXSignal(raw: RawTwitterXSignal, index: number): LumoraSignal | null {
  const id = String(raw.id || "").trim() || `twitter_x_${index}`;
...
async function loadFixture(): Promise<LumoraSignal[]> {
  ...
  const raw = JSON.parse(json) as RawTwitterXSignal[];
  return raw
    .map((item, index) => normalizeTwitterXSignal(item, index))
    .filter((item): item is LumoraSignal => !!item);
}

function buildProviderRequest(limit: number): { url: string; headers: Record<string, string> } {
  const url = process.env.TWITTER_X_SIGNAL_PROVIDER_URL?.trim();
  const token = process.env.TWITTER_X_SIGNAL_PROVIDER_TOKEN?.trim();

  if (!url) {
    throw new Error("TWITTER_X_SIGNAL_PROVIDER_URL is not configured");
  }
...
export async function ingestTwitterXSignals(
  options: TwitterXIngestOptions = {}
): Promise<{
  ok: boolean;
  ...
  try {
    ...
    const payload = await fetchJsonWithTimeout<RawTwitterXSignal[]>(request.url, {
      method: "GET",
    ...
    if (!useFixtureOnFailure) {
      return {
        ...
        count: 0,
        signals: [],
        error: error instanceof Error ? error.message : "unknown_twitter_x_ingest_error",
      };
    }
