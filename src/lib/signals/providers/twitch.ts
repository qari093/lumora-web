...
type TwitchIngestOptions = {
  limit?: number;
  useFixtureOnFailure?: boolean;
};

const FIXTURE_PATH = path.join(process.cwd(), "src/lib/signals/fixtures/twitch.sample.json");
...
function normalizeTwitchSignal(raw: RawTwitchSignal, index: number): LumoraSignal | null {
  const id = String(raw.id || "").trim() || `twitch_${index}`;
...
async function loadFixture(): Promise<LumoraSignal[]> {
  ...
  const raw = JSON.parse(json) as RawTwitchSignal[];
  return raw
    .map((item, index) => normalizeTwitchSignal(item, index))
    .filter((item): item is LumoraSignal => !!item);
}

function buildProviderRequest(limit: number): { url: string; headers: Record<string, string> } {
  const url = process.env.TWITCH_SIGNAL_PROVIDER_URL?.trim();
  const token = process.env.TWITCH_SIGNAL_PROVIDER_TOKEN?.trim();

  if (!url) {
    throw new Error("TWITCH_SIGNAL_PROVIDER_URL is not configured");
  }
...
export async function ingestTwitchSignals(
  ...
  try {
    ...
    const payload = await fetchJsonWithTimeout<RawTwitchSignal[]>(request.url, {
      method: "GET",
    ...
    if (!useFixtureOnFailure) {
      return {
        ...
        count: 0,
        signals: [],
        error: error instanceof Error ? error.message : "unknown_twitch_ingest_error",
      };
    }
