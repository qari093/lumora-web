...
type NewsRssIngestOptions = {
  limit?: number;
  useFixtureOnFailure?: boolean;
};

const FIXTURE_PATH = path.join(process.cwd(), "src/lib/signals/fixtures/news_rss.sample.json");
...
function normalizeNewsRssSignal(raw: RawNewsRssSignal, index: number): LumoraSignal | null {
  const id = String(raw.id || "").trim() || `news_rss_${index}`;
...
async function loadFixture(): Promise<LumoraSignal[]> {
  ...
  const raw = JSON.parse(json) as RawNewsRssSignal[];
  return raw
    .map((item, index) => normalizeNewsRssSignal(item, index))
    .filter((item): item is LumoraSignal => !!item);
}

function buildProviderRequest(limit: number): { url: string; headers: Record<string, string> } {
  const url = process.env.NEWS_RSS_PROVIDER_URL?.trim();
  const token = process.env.NEWS_RSS_PROVIDER_TOKEN?.trim();

  if (!url) {
    throw new Error("NEWS_RSS_PROVIDER_URL is not configured");
  }
...
export async function ingestNewsRssSignals(
  options: NewsRssIngestOptions = {}
): Promise<{
  ok: boolean;
  ...
  try {
    ...
    const payload = await fetchJsonWithTimeout<RawNewsRssSignal[]>(request.url, {
      method: "GET",
    ...
    if (!useFixtureOnFailure) {
      return {
        ...
        count: 0,
        signals: [],
        error: error instanceof Error ? error.message : "unknown_news_rss_ingest_error",
      };
    }
