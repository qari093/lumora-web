...
type TikTokIngestOptions = {
  limit?: number;
  useFixtureOnFailure?: boolean;
};

const FIXTURE_PATH = path.join(process.cwd(), "src/lib/signals/fixtures/tiktok.sample.json");
...
function normalizeTikTokSignal(raw: RawTikTokSignal, index: number): LumoraSignal | null {
  const id = String(raw.id || "").trim() || `tiktok_${index}`;
...
async function loadFixture(): Promise<LumoraSignal[]> {
  ...
  const raw = JSON.parse(json) as RawTikTokSignal[];
  return raw
    .map((item, index) => normalizeTikTokSignal(item, index))
    .filter((item): item is LumoraSignal => !!item);
}

function buildProviderRequest(limit: number): { url: string; headers: Record<string, string> } {
  const url = process.env.TIKTOK_SIGNAL_PROVIDER_URL?.trim();
  const token = process.env.TIKTOK_SIGNAL_PROVIDER_TOKEN?.trim();

  if (!url) {
    throw new Error("TIKTOK_SIGNAL_PROVIDER_URL is not configured");
  }
...
export async function ingestTikTokSignals(
  options: TikTokIngestOptions = {}
): Promise<{
  ok: boolean;
  ...
  try {
    ...
    const payload = await fetchJsonWithTimeout<RawTikTokSignal[]>(request.url, {
      method: "GET",
    ...
    if (!useFixtureOnFailure) {
      return {
        ...
        count: 0,
        signals: [],
        error: error instanceof Error ? error.message : "unknown_tiktok_ingest_error",
      };
    }
