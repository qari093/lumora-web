...
/** Loads a real GLB if present; otherwise shows a stylized capsule fallback */
function HeroGLTF({
  name,
  position = [0, 9, 220],
  scale = 2.4,
...
// Preload (safe no-ops if files are missing)
try {
  ...
  React.useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === " ") {
        ...
        const start = heroRef.current?.position ?? new THREE.Vector3(0, 9, 220);
...
export default function Game3DPlus() {
  ...
  const [currentHero, setCurrentHero] = React.useState<HeroName>(
    () => (localStorage.getItem("rbn:hero3d") as HeroName) || "Aurora"
  );
  ...
  React.useEffect(() => {
    localStorage.setItem("rbn:hero3d", currentHero);
  }, [currentHero]);

  React.useEffect(() => {
    getBalance().then(setHud);
    startSession().then((s) => setSession({ id: s.sessionId }));
  }, []);
