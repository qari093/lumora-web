...
export async function POST(req: Request) {
  try {
    const auth = await bearerUid();
    if (!auth.ok) return NextResponse.json({ ok: false, error: auth.error }, { status: 401 });
    const uid = auth.uid;
    if (!(await limit(uid, 'COMMENT')))
      return NextResponse.json({ ok: false, limited: true }, { status: 429 }); // reuse COMMENT budget (gentle)
    ...
    if ((me.wallet.energy || 0) <= 0)
      return NextResponse.json({ ok: false, error: 'insufficient_energy' }, { status: 400 });
