...
import { getFypSourceHealthSnapshots, type FypSourceHealthSnapshot } from './sourceHealth';
...
export function selectFypSources(input: FypSourceSelectionInput = {}): FypSourceSelectionResult {
  ...
  const candidates = FYP_SOURCE_REGISTRY.filter(
    (source) => !input.category || source.category === input.category,
  )
    .filter((source) => {
    ...
    .flatMap((source) => {
      ...
      return sourceHealth ? [{ source, health: sourceHealth }] : [];
    });

  const rejected = candidates
    .filter(
      (entry) =>
        !entry.health.eligible ||
...
export function getFypSourceFailoverChain(primarySourceId: string): FypSourceRegistryItem[] {
  ...
  const globalFallbacks = selectFypSources({ maxSources: 6 }).selected.filter(
    (source) =>
      source.id !== primary.id && !sameCategory.some((candidate) => candidate.id === source.id),
  );
...
export function validateFypSourceSelectionFailoverRuntime(): boolean {
  ...
  return (
    ...
    embedSelection.selected.every(
      (source) => source.ingestionMode.includes('embed') || source.ingestionMode.includes('link'),
    ) &&
