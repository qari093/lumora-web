...
echo "▶️ Step ${STEP}/${TOTAL} — API headers regression (prod)"
echo "──────────────────────────────────────────────────────────────"
...
LOG="/tmp/lumora_step57_prod_${PORT}.log"
SV_PID=""
...
OUT="/tmp/step57_api_headers_regression.txt"
: >"$OUT"
fail=0

echo
echo "• HEAD checks (API) (follow redirects)"
for r in $routes; do
  rr="$(echo "$r" | tr -d ' ')"
  [ -n "$rr" ] || continue
  echo "— ${rr} —" | tee -a "$OUT"
