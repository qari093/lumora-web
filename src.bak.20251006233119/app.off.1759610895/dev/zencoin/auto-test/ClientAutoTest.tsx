...
// client-only widgets
const StakingDashboard = dynamic(() => import("@/components/lumora/StakingDashboard"), { ssr: false });
const ReferralManager  = dynamic(() => import("@/components/lumora/ReferralManager"),  { ssr: false });

// mount-gate to avoid any SSR/CSR mismatch
function Mounted({children}:{children:React.ReactNode}){
  const [ok, setOk] = useState(false);
  useEffect(() => { setOk(true); }, []);
  if(!ok){
    return (
      <div suppressHydrationWarning style={{ padding:20, background:"#000", color:"#999", minHeight:"100vh" }}>
        Loading…
      </div>
    );
  }
...
function AutoRenderer() {
  ...
  const pill = (s:"done"|"active"|"pending"): React.CSSProperties => ({
    ...
    background: s==="done" ? "green" : s==="active" ? "orange" : "#444", color:"#fff"
  });

  return (
    <div suppressHydrationWarning style={{ padding:20, background:"#000", color:"#fff", minHeight:"100vh" }}>
      <div style={{background:"limegreen", color:"#000", fontWeight:900, padding:"8px 12px", borderRadius:6, marginBottom:12}}>
        ✅ Client Rendered OK
      </div>
...
export default function ClientAutoTest() {
  return (
    <BalanceProvider>
      <Mounted>
        <AutoRenderer />
