/** Zen Economy - Offline-first (Phase 3, hardened) */
...
/* Safe number parser */
function sane(n: unknown, d=0){ const x = Number(n); return Number.isFinite(x) ? x : d; }

export function loadCoins(): number {
  try { return sane(JSON.parse(localStorage.getItem(K_COINS) || "0"), 0); } catch { return 0; }
}
export function saveCoins(n: number) {
  try { localStorage.setItem(K_COINS, JSON.stringify(Math.max(0, sane(n,0)))); } catch {}
}
export function addCoins(n: number) {
  const next = Math.max(0, loadCoins() + sane(n,0));
  saveCoins(next);
  const inv = loadInv(); inv.coins = next; saveInv(inv);
  return next;
}
export function spendCoins(n: number): number | null {
  const want = Math.max(0, sane(n,0));
  const cur  = Math.max(0, loadCoins());
  if (cur < want) return null;                  // allow equal (>=)
  const next = cur - want;
  saveCoins(next);
  const inv = loadInv(); inv.coins = next; saveInv(inv);
  return next;
...
export function loadInv(): Inventory {
  try {
    ...
    if (raw) {
      const v = JSON.parse(raw) as Inventory;
      if (typeof v?.coins === "number" && Array.isArray(v?.items)) return v;
    }
  } catch {}
...
export function addItem(id: string, title: string, qty = 1): Inventory {
  ...
  else inv.items.push({ id, title, qty: Math.max(1, qty) });
  saveInv(inv);
  return inv;
}
...
export function saveReceipt(r: Receipt) {
  ...
  all.unshift(r);
  try { localStorage.setItem(K_RCPTS, JSON.stringify(all)); } catch {}
}
